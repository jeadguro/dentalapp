...
export const getPatientQR = async (req, res, next) => {
  try {
    ...
    // Si el paciente no tiene código de acceso (ej: se auto-registró con password)
    if (!patient.accessToken || !patient.accessCode) {
      return res.json({
        success: true,
        data: {
          patient: {
            ...
            name: patient.name,
            email: patient.email,
            selfRegistered: patient.selfRegistered
          },
          qr: null,
          accessLink: null,
          accessCode: null,
          message: patient.selfRegistered 
            ? 'Este paciente se registró con email y contraseña' 
            : 'Este paciente no tiene código de acceso'
        }
      });
    }

    // URL de acceso directo
    const accessLink = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/portal/login?token=${patient.accessToken}`;
