...
export default function Dashboard() {
  ...
  const colorClasses = { dental: 'bg-dental-100 text-dental-600', blue: 'bg-blue-100 text-blue-600', amber: 'bg-amber-100 text-amber-600', green: 'bg-green-100 text-green-600' };
  ...
  return (
    <div className="page-container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <Link to="/patients/new" className="btn-primary justify-center"><UserPlus className="w-5 h-5" />Nuevo Paciente</Link>
        <Link to="/consultations/new" className="btn-secondary justify-center"><Plus className="w-5 h-5" />Nueva Consulta</Link>
        <Link to="/appointments/new" className="btn-secondary justify-center"><Calendar className="w-5 h-5" />Nueva Cita</Link>
      </div>

      <div className="card p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-display font-semibold text-charcoal-900">Citas de Hoy</h2>
          <Link to="/appointments" className="btn-ghost text-sm">Ver todas<ArrowRight className="w-4 h-4" /></Link>
        </div>
          ...
          <div className="space-y-3">
            {todayAppointments.slice(0, 5).map((apt) => (
              <div key={apt._id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50">
                <div className="w-12 h-12 rounded-xl bg-dental-100 flex items-center justify-center"><Clock className="w-5 h-5 text-dental-600" /></div>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-charcoal-800 truncate">{apt.patient?.name}</p>
