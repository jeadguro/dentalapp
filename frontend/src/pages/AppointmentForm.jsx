...
export default function AppointmentForm() {
  ...
  const [form, setForm] = useState({ patient: patientId || '', date: formatForInput(new Date()), type: 'checkup', notes: '' });
  ...
  return (
    <div className="page-container max-w-2xl">
      <Link to="/appointments" className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-700 mb-6"><ArrowLeft className="w-4 h-4" />Volver</Link>
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        ...
        <div>
          ...
          <textarea value={form.notes} onChange={(e) => setForm({...form, notes: e.target.value})} className="input-field min-h-[80px]" placeholder="Indicaciones especiales..." />
        </div>
        <div className="flex justify-end gap-3 pt-4 border-t">
          <Link to="/appointments" className="btn-secondary">Cancelar</Link>
          <button type="submit" disabled={saving} className="btn-primary">{saving ? 'Guardando...' : <><Save className="w-5 h-5" />Crear Cita</>}</button>
        </div>
      </form>
    </div>
  );
